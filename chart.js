...
function injectChartCSS() {
  ...
  style.textContent = `
    ...
    #bar-chart-overlay svg {
      width: 100%;
      height: 36px;                  /* REDUCED: was 42px - makes bars shorter/more compact */
    ...
    @media screen and (max-width: 768px) {
      ...
      #bar-chart-overlay svg {
        height: 32px;                /* REDUCED: was 36px - more compact on mobile */
      }
      ...
      .percent-label {
        font-size: 6px !important;   /* REDUCED: was 7px */
      }
    }
    ...
    @media screen and (min-width: 769px) and (max-width: 1024px) {
      ...
      .percent-label {
        font-size: 7px !important;   /* REDUCED: was 7.5px */
      }
    }
    ...
    @media screen and (min-width: 1400px) {
      ...
      .percent-label {
        font-size: 7.5px !important; /* REDUCED: was 9px */
      }
    }
...
const detectionData = {
  ...
  "chapter4":  [90514,  480,  180147,  1544,  1748,  2183],
  ...
  "chapter9":  [1389, 1234, 1178, 689,  434,  1800],
  "chapter10": [1623, 1478, 1445, 834,  555,  2100],
  "chapter11": [1890, 1689, 1667, 967,  656,  2400],
  "chapter12": [2234, 1923, 1953, 1134, 756,  2800]
  // REMOVED: "conclusion"
};
...
function calculateBarWidths(data, containerWidth) {
  ...
  const percentFontSize = isMobile ? 6 : isTablet ? 7 : 7.5;   // REDUCED
...
function initOverlayChart() {
  ...
  if (container.select("svg").empty()) {
    const svg = container.append("svg")
      .attr("viewBox", "0 0 750 36")  // REDUCED: was 900x42 - makes chart more compact
...
function updateOverlayChart(targetData, chapter = 'intro') {
  ...
  const height = 36;  // REDUCED: was 42
  
  // Responsive height adjustment
  const isMobile = window.innerWidth <= 768;
  const isTablet = window.innerWidth > 768 && window.innerWidth <= 1024;
  const responsiveHeight = isMobile ? 32 : isTablet ? 34 : height;  // REDUCED
  ...
  function animate(currentTime) {
    ...
    mainTexts.merge(mainTextsEnter)
      ...
      .attr("y", responsiveHeight / 2 - (isMobile ? 2 : 3))  // REDUCED spacing
      .attr("fill", "#ffffff")
    ...
    // Update percentage labels with responsive sizing
    ...
    percentTexts.merge(percentTextsEnter)
      ...
      .duration(300)
      .attr("x", d => d.x + d.width / 2)
      .attr("y", responsiveHeight / 2 + (isMobile ? 4 : 5))  // REDUCED spacing
      .attr("fill", d => d.color)
      .attr("font-size", `${isMobile ? 6 : isTablet ? 7 : 7.5}px`)  // REDUCED
      .attr("opacity", 0.9);
...
function pulseChart() {
  ...
  const isMobile = window.innerWidth <= 768;
  const isTablet = window.innerWidth > 768 && window.innerWidth <= 1024;
  const height = isMobile ? 32 : isTablet ? 34 : 36;  // REDUCED
...
if (window.location.hostname === 'localhost') {
  ...
  configPanel.innerHTML = `
    <h4 style="margin: 0 0 0.5rem; color: #00ffff;">Chart Config (Compact)</h4>
    ...
    <div style="margin-top: 0.5rem; color: #66b3ff; font-size: 0.7rem;">
      ...
      Size: <span style="color: #ffb366;">750x36 (Compact)</span>
    </div>
  `;
