// animate2.js - Rubymar Animation (Simplified popups)
...
(function(global) {
  ...
  const chapter8Points = [
    {
      ...
      popupHtml: `
        <div class="enhanced-popup">
          <img src="rubymar15feb.png" class="annotation-img" alt="Rubymar Sat 2">
        </div>
      `,
      offset: [-60, -40],
      delay: 500,
    ...
    },
    {
      ...
      popupHtml: `
        <div class="enhanced-popup">
          <img src="rubymarmar1.png" class="annotation-img" alt="Rubymar Sat 3">
        </div>
      `,
      offset: [-170, 170],
      delay: 800,
  ...
  async function animateRubymarPath(map) {
    try {
      const resp = await fetch('rubymar_path.geojson');
      if (!resp.ok) {
  ...
  /**
   * Enhanced final text annotation
   */
  function addChapter8TextAnnotation(map) {
    if (chapter8TextMarker) return;

    const el = document.createElement('div');
    el.className = 'annotation-text sinking-annotation';
    el.innerHTML = `
      <div class="sinking-text">${textAnnotationContent}</div>
    `;
  ...
  // Add enhanced styles (keeping all animations, simplified popups)
  if (!document.getElementById('chapter8-styles')) {
    ...
    style.textContent = `
      ...
      /* Critical popup with red glow - KEEPING THIS */
